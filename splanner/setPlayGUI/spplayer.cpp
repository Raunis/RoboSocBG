...
SpPlayer::SpPlayer(int n, SpSetPlay * s, QString na ) :
    ...
    ghost(false), identificatedByNumber(false), moving(false), participantOpponent(false)
{
    actualAction = NULL;
...
void SpPlayer::gaveTheBallFromStep(SpStep *spStep) //CONTROLA A POSSE DE BOLA NA TRANSIÇÃO DE STEP
{
    ...
    foreach(SpTransition * spTransition, spStep->getSourceTransitionsList())
    {
        ...
        else
        {
            switch (action->getType())
            {
                /************************************
                 *         AÇÃO DE PASSE            *
                 *   Altera a posse de bola para    *
                 *   o jogador selecionado como     *
                 *   destpoint da ação de passe     *
                 *   (altera o ballholder           *
                 *   do nextstep)                   *
                 ***********************************/
                //@@_newAction_comPasse
                ...
                /////////////////////////////////////
                //_________FINAL DO GERADOR________//
                /////////////////////////////////////
                ...
                /************************************
                 *         AÇÃO SEM PASSE           *
                 *   Define o jogador atual como    *
                 *   possesor da bola no próximo    *
                 *   passo                          *
                 *   (mantém o ballholder           *
                 *   no nextstep)                   *
                 ***********************************/
...
void SpPlayer::updateStep(SpStep *spStep)
{
    ...
    foreach(SpTransition * spTransition, spStep->getSourceTransitionsList())
    {
        ...
        else
        {
            if(ballOwnerOnSpStep)
            {
                switch (action->getType())
                {
                    ...
                    case SpAction::goToOffsideLine:
                        ...
                        break;
                    //@@_newAction_ADICIONAR QUAIS TIPOS DE AÇÃO? ^
                ...
                {
                    if( 
                        action->getType() != SpAction::pass
                     && action->getType() != SpAction::passForward
                     && action->getType() != SpAction::shoot
                        //@@_newAction_ballStoppedOnStep
            ...
            else
            {
                switch (action->getType())
                {
                    ...
                    //@@_newAction_actionsByTransitions
        ...
        else
        {
            switch (action->getType())
            {
                // move para o destino desejado na ação
                /**************************************
                 * CONTROLA A MOVIMENTAÇÃO DO JOGADOR *
                 *        NA TRANSIÇÃO DE STEP        *
                 **************************************
                 *Aciona o método que altera a posição*
                 * do jogador em campo, e irá movê-lo *
                 *  para o jogador para o destpoint   *
                 *            selecionado             *
                 **************************************/
                //@@_newAction_movePosition
                 ...
                 *        NA TRANSIÇÃO DE STEP        *
                 **************************************
                 *Aciona o método que mantém a posição*
                 *do jogador no mesmo lugar do step   *
                 *  atual durante a transição para o  *
                 *          próximo step              *
                 **************************************/
                //@@_newAction_keepPosition
...
void SpPlayer::actualize(SpStep *step, SpTransition *transition, bool ballHolder)
{
    ...
    if(positionsByStep.contains(stepId))
    {
        ...
        lastPosition = new SpPoint(actualPosition);
        participant = true;
       // participantOpponent=true;//testecarol
...
void SpPlayer::addAction(SpAction * newAction)
{
    ...
    if(newAction->getType() != SpAction::hold &&
             ...
             *       JANELA DE TRANSIÇÃO        *
             ************************************
             *   Ativa a janela de transição    *
             *   quando um novo comportamento   *
             *   é selecionado no SPlanner      *
             *                                  *
             ************************************/
            //@@_newAction_Dialog
    ...
    //@@_newAction_conditionsByTransitions
...
void SpPlayer::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
    ...
    /*************************************
     * CRIAÇÃO INICIAL DO MENU DE AÇÕES  *
     *************************************
     *  Todas as ações são adicionadas   *
     *inicialmente e depois são removidas*
     *    de acordo com as situações     *
     *     específicas de uma jogada     *
     *************************************/
    //@@_newAction_Menu
    ...
    //aqui tem dois pontos de inserção, dentro e fora do foreach
    //@@_newAction_passRegion_menu
    ...
    if( ! ghost)
    {
        ...
        if( ! currentBallHolder )
        {
            ...
            /******************************************
             * CONSTRUÇÃO DO MENU DE AÇÕES REALIZADAS *
             *            SEM POSSE DE BOLA           *
             ******************************************
             *   remove as ações que são realizadas   *
             *           COM posse de bola            *
             *                                        *
             ******************************************/
            //@@_newAction_Ball
        ...
        {
            //@@_newAction_insertMenu_passRegion
            ...
            if( situation != play_on && setPlay->isBallStopped())// se é play on ou bola parada
            {
               ...
               if(situation != keeper_catch && situation != kick_off)
    ...
    {
        /*************************************
         *       MENU DO JOGADOR GHOST       *
         *************************************
         *  Todas as ações são removidas     *
         *  quando o jogador se encontra no  *
         *          estado de ghost          *
         *************************************/
        // @@_newAction_ghost
    ...
    /*********************************************
     *         CHAMADA DO MÉTODO addAction       *
     *********************************************
     *   Instancia uma ação para um jogador:     *
     *                                           *
     *   O método addAction da classe spplayer   *
     * define a coordenada atual do jogador em   *
     * campo como destpoint da ação, é utilizado *
     *  para comportamentos que não alteram a    *
     *      posição do jogador ou da bola,       *
     *  com exceção das actions de lance à gol,  *
     * instanciadas pelo addAction da classe     *
     *spplayer, que irá calcular autmaticamente  *
     *   como destino da ação o centro do gol    *
     *                                           *
     *   O método addAction da classe spfield    *
     *  utiliza como destpoint o ponto que foi   *
     *  clicado no campo como destino da ação, é *
     * utilizado para comportamentos que alteram *
     * a posição do jogador ou da bola no campo  *
     *                                           *
     *  addAction é responsável por invocar um   *
     *     método que desenha a ação na tela     *
     *********************************************/
    // @@_newAction_genericDraw
    ...
    else
    {
        ...
        //aqui precisa usar no foreach a mesma playerList declarada em //@@ newAction menu passRegion
        //o código aqui vai ser o mesmo das 3 linhas de código acima (foreach, if e field)
            
        //@@_newAction_passRegion_genericDraw
...
void SpPlayer::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *widget)
{
    // paint serve para realizar desenhos
    painter->setPen(Qt::NoPen);
//faz a bola aparecer no campo!!!!
    if(currentBallHolder)
    {
        //QRect :: QRect ( int x , int , int largura , int altura )
        //Constrói um retângulo com ( x , y ) como seu canto superior esquerdo e a largura e a altura especificadas.

        QRect target(0, 1, 3, 3);// parametros 2 serve para dimensionar a ṕosiçâo da bola em relação ao jogador
        QPixmap image("./setPlayGUI/images/ball.png");//A classe QPixmap é uma repr
                                                        //esentação de imagem fora da tela que pode ser usada como um dispositivo de pintura.
        painter->drawPixmap(target, image);
    }


    if(participantOpponent)
    {
        QRect target(-2, 0, 5, 5);
        QPixmap image("./setPlayGUI/images/opponentsshirt.png");
        painter->drawPixmap(target, image);
    }
    else {
   ...
   if (participantOpponent)
    {
         painter->drawText(QRect(-0.5,1,2,2.5), Qt::AlignCenter, QString("%1").arg(number));
    }


   else if (participant)
    {
        // enumera o jogador que ja começa com o kickoff
       painter->drawText(QRect(0.5,-1,2,2.5), Qt::AlignCenter, QString("%1").arg(number));
    } else {
    // inumeros os jogadores que estâo esperando para serem participantes
        painter->drawText(QRect(-0.5,1,2,2.5), Qt::AlignCenter, QString("%1").arg(number));
    }
    ...
    {
        QFont font;// edita o tamanho da fonte
        font.setBold(true);
        painter->setFont(font);
        QString simb;
...
void SpPlayer::removeFromAllSetPlay()
{
    ...
    conditionsByTransitions.clear();
    actived = false;
    ctrlPressed = false;moving  =false;
