...
{
    type = ui->offensiveTypeRB->isChecked() ? off : def;// se ui->offensiveTypeRB->isChecked() for true type =off (0)
                                                        //                                    se for false type= def (1)
    situation = ui->situationComboBox->currentIndex() - 1;
    specialAreas.clear();

    if(!ui->offensiveTypeRB->isChecked() && !ui->defensiveTypeRB->isChecked())//se nenhum tipo for escolhido (ofensivo ou defensivo)
    {
        QMessageBox::warning(this, "No type selected",
                             "Choose if the setplay is on offensive or defensive situation.");
    }
    else if(ui->situationComboBox->currentIndex() == 0)//nenhuma opçâo do menu foi escolhida
    {
        QMessageBox::warning(this, "No situation selected",
                             "Choose the setplay's trigger situation.");
    }
    else if(ui->positionGroupBox->isEnabled())// independente de ser ofensivo ou defensivo e das opçôes de menu, isso sempre será feito para ambos
    {
        if(ui->pointBox->isChecked())
            specialAreas.append(point);// a caixa d epontos esta clicada
        else
        {
            if(ui->fieldBox->isChecked())// a caixa de campo ta ligada?
                specialAreas.append(field);
            else
            {
                if(ui->ourMiddleField->isChecked())// (nosso meio campo) se está opção esta ligada desabilito as demais do else
                    specialAreas.append(our_middle_field);
                else
                {
                    if(ui->ourBack->isChecked())//nossa volta
                        specialAreas.append(our_back);
                    if(ui->ourMiddle->isChecked())//seu meio
                        specialAreas.append(our_middle);
                    if(ui->ourFront->isChecked())//nossa frente
                        specialAreas.append(our_front);
                }

                if(ui->theirMiddleField->isChecked())//o campo do meio
                    specialAreas.append(their_middle_field);
                else
                {
                    if(ui->theirFront->isChecked())//sua frente
                        specialAreas.append(their_front);
                    if(ui->theirMiddle->isChecked())//seu meio
                        specialAreas.append(their_middle);
                    if(ui->theirBack->isChecked())// suas costas
                        specialAreas.append(their_back);
                }

                if(ui->leftCB->isChecked()){//esquerda
                    //specialAreas.append(left_region);
                    specialTransverseAreas.append(left_region);//JAF2014
                }
                else
                {
                    if(ui->farLeft->isChecked()){//esquerda longínqua
                        //specialAreas.append(far_left);
                        specialTransverseAreas.append(far_left);//JAF2014
                    }
                    if(ui->midLeft->isChecked()){//meio esquerda
                        //specialAreas.append(mid_left);
                        specialTransverseAreas.append(mid_left);//JAF2014
                    }
                    if(ui->cenLeft->isChecked()){//centro esquerda
                        //specialAreas.append(centre_left);
                        specialTransverseAreas.append(centre_left);//JAF2014
                    }
                }

                if(ui->rightCB->isChecked()){//direita
        ...
        if(situation == throw_in && (   !specialTransverseAreas.contains(left_region) && !specialTransverseAreas.contains(right_region) ) )
        {
