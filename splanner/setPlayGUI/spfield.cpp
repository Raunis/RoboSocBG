...
SpField::~SpField()
{
    foreach (SpPlayer *p, inPlayers)
        scene()->removeItem(p);
    inPlayers.clear();
    foreach (SpPlayer *p1, outPlayers)
        scene()->removeItem(p1);
    outPlayers.clear();
    setPlay = NULL;
}
...
void SpField::setSituationAndRegion(int sit, QList <int> reg, QList <int> transverse_reg)//VERRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRs
{
    situation = sit;
    specialAreasDef = reg;
   ...
   QMap<int, SpPlayer *> playersOpponent = setPlay->getPlayersOpponent();
     for(int i = 1; i < 12; i++)
    {
       SpPlayer *player2 = playersOpponent[i];
       outPlayersOpponent.append(player2);
...
void SpField::changePlayers(QList <SpPlayer *> players, QList <SpPlayer *> playersOpponent)
{
    foreach(SpPlayer *p , players)
        removePlayerFromBench(p);
     foreach(SpPlayer *p2 , playersOpponent)
        removePlayerFromBenchOpponent(p2);
}


void SpField::removePlayerFromBench(SpPlayer *p)
{
    if(outPlayers.contains(p))//chega se existe o jogador respctivo
    {
        if(p->getAction())
           scene()->addItem(p->getAction());
            outPlayers.removeOne(p);
       inPlayers.append(p);// fica la em cima o jogador

    }
}
void SpField::removePlayerFromBenchOpponent(SpPlayer *p)
{
    if(outPlayersOpponent.contains(p))//chega se existe o jogador respctivo
    {
        if(p->getAction())
            scene()->addItem(p->getAction());
            outPlayersOpponent.removeOne(p);
        inPlayersOpponent.append(p);
    }
}
//banco dos agentes
...
////banco dos agentes adversários
void SpField::playerToBenchOpponent(SpPlayer *p)
{
    int id = p->getNumber();

    int width = benchRectOpponent.width();
    int playerSize = 30*width/510;
    int spaceSize = (width - 11*playerSize) / 12;
    p->setPos(benchRectOpponent.left()+ spaceSize*(id-1) + playerSize*id , benchRectOpponent.center().y() );
    //p->setPos(benchRect.center().x(), benchRect.top() + spaceSize*(id-1) + playerSize*id );

    if(!outPlayersOpponent.contains(p))
    {
        if(p->getAction())
            scene()->removeItem(p->getAction());
        outPlayersOpponent.append(p);
        inPlayersOpponent.removeOne(p);
    }
}
...
/////////////////////////////////////
// ________FINAL DO GERADOR________//
/////////////////////////////////////
...
void SpField::cleanField()
{
    ...
    foreach(SpPlayer *player, inPlayersOpponent)
        playerToBenchOpponent(player);

}

void SpField::actualize(SpStep * step)
{
    // se o agente está sendo usado no step remova ele do banco de agente
...
void SpField::calculateRects()
{
    ...
    benchRectOpponent = QRect( left_x + scaleToField((pitch_length * 0.9 - bench_length)),
                          top_y  - scaleToField(bench_width*2),
                          scaleToField(bench_length), scaleToField(bench_width) );
    ...
    else if(situation == kick_off)//area especial de kick_off
    {
...
{
    replaceBenchPlayers();
    replaceBenchPlayersOpponent();
    replaceParticipantPlayers();
    replaceParticipantPlayersOpponent();
}

//constroi o retangulo dos jogadores
...
//CRIADO PARA O OPONENTE
void SpField::replaceBenchPlayersOpponent()
{
    ...
    foreach(SpPlayer *player, outPlayersOpponent)
    {
        int n = player->getNumber();
        player->setScale(getScale());
        player->setPos(benchRectOpponent.right()+ spaceSize*(n-33) + playerSize*n , benchRectOpponent.center().y() );
    }
}
...
void SpField::replaceParticipantPlayersOpponent()
{
       foreach(SpPlayer *player, inPlayersOpponent)
        player->updatePosOnField(getScale());
}
...
void SpField::drawFieldLines(QPainter *painter)
{
    ...
    painter->drawRect(benchRectOpponent);
    ...
    if( drawingAction != NULL )
    {
         ...
         if(e->button() == Qt::LeftButton)
        {
            ...
            if(actType == SpAction::pass
                ...
                //@@_newAction_field_EventCondition
                ...
                )
            {
                ...
                else
                {
                    ...
                    if(line->length() <= SpAction::maxBallDistance)
                    {
                        ...
                        //@@_newAction_PassRegion_fieldEvent
