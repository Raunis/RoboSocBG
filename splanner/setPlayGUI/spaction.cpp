    ...
    originalPaintSize(5)
{
    ...
    /**************************************
     *CALCULA A COORDENADA DE LOCALIZAÇÃO *
     *         DO CENTRO DO GOL           *
     **************************************
     *  Define o destPoint das açẽos de   *
     *          lance à gol               *
     **************************************/
    //@@_newAction_goalCenter
...
QString SpAction::print()
{
    ...
    //tem que ter um código para o pass, outro pro passforward e ações de movimento, outro para ações locais e de chute ao gol. 
    //@@_newAction_passRegion
...
void SpAction::actualize()
{
    ...
    if(destActor )
    {
        ...
        /**************************************
         *ATUALIZA O TIPO DE UMA AÇÃO DE PASSE*
         **************************************/
        //@@_newAction_Passe
        ...
        else if(destActor->getAction() && destActor->getAction()->getType() == run)
            type = passForward;
...
void SpAction::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    /************************************
    |                                   |
    |  Chamada de impressão para ações  |
    |   com o sourcePoint = destPoint   |
    |                                   |
    ************************************/
    //@@_newAction_Local
    ...
    QPointF edgeOffset((line.dx() * paintSize) / length, (line.dy() * paintSize) / length);

    QPointF falseDestPoint = destPoint;

    if(type == pass && length > paintSize*2)
    ...
    QPointF destArrowP1 = line.p2() + QPointF(sin(angle - Pi / 3) * paintSize/2,
                                                  cos(angle - Pi / 3) * paintSize/2);
    QPointF destArrowP2 = line.p2()  + QPointF(sin(angle - Pi + Pi / 3) * paintSize/2,
                                                  cos(angle - Pi + Pi / 3) * paintSize/2);
    switch (type)
    {
        case shoot:
        {
...
void SpAction::drawPass(QPainter *painter, QLineF &line, QPointF &destArrowP1, QPointF &destArrowP2)
{
    ...
    painter->setPen(QPen(Qt::black, paintSize/6, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
...
void SpAction::drawShoot(QPainter *painter, QPointF falseSourcePoint, QPointF &destArrowP1, QPointF &destArrowP2)
{
    ...
    painter->setPen(QPen(Qt::white, paintSize/6, Qt::DotLine, Qt::RoundCap, Qt::RoundJoin));
 ...
 *     Específico para cada ação      *
            O desenvolvedor
  depois pode alterar esse método para
   criar um desenho melhor para o novo
            comportamento
 **************************************/
...
{
    painter->setBrush(Qt::black);
    painter->setPen(QPen(Qt::black, paintSize/6, Qt::DotLine, Qt::RoundCap, Qt::RoundJoin));
    painter->drawLine(line);
    painter->drawPolygon(QPolygonF() << line.p2() << destArrowP1 << destArrowP2);
}
