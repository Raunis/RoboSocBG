...
namespace fcportugal{
  namespace setplay{
    ...
    public:
      InitMessage(double time, int setplayNumber_, vector<const string*>* arguments_)
			  :SetplayMessage(time), setplayNumber(setplayNumber_), arguments(arguments_){}

      InitMessage(double time, int setplayNumber_, vector<unsigned int>* players);

      InitMessage(const double time, string message);
  
      virtual ~InitMessage(){// Estava a dar bronca, experiencia //delete arguments;
      };
